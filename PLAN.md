...
| Step 4B: CLI dry-run wiring to engine metadata | `PENDING` | — | Remove hardcoded stage string in `run --dry-run`; render stage order from orchestrator/core metadata. |
...
Current status: foundational steps (1-4A) are merged, and pending work is now specified at implementation depth. The next executable atomic step is Step 4B.
