...
func TestFixtureCaseAgentContextQuality(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			requiredConstraint: "Prefer existing repository commands or Python entrypoints to materialize",
			requiredForbidden:  "Do not add durable helper scripts outside .concierge",
		},
		{
			name:               "preprocess",
			caseID:             "mnist_missing_preprocess",
			stepID:             core.EnsureStepPreprocessContract,
			expectedSections:   []string{"preprocess_contract"},
